...
async function runMigrations() {
  ...
  if (!databaseUrl) {
    logger.error('DATABASE_URL is not set in process.env. Cannot run migrations. Make sure it is set in your .env file and loaded correctly.');
    if (envConfig.parsed && envConfig.parsed.DATABASE_URL) {
      logger.info(`DATABASE_URL found in parsed .env: ${envConfig.parsed.DATABASE_URL}`);
    } else {
      logger.warn('DATABASE_URL not found in parsed .env or .env was not parsed.');
    }
    process.exit(1);
  }

  logger.info(`Using DATABASE_URL: ${databaseUrl.substring(0, databaseUrl.indexOf('@') + 1)}...`); // Log URL without credentials
