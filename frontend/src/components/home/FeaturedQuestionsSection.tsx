...
const FeaturedQuestionsSection = ({ questions }: FeaturedQuestionsSectionProps) => {
  ...
  const scroll = (direction: 'left' | 'right') => {
    ...
    const cardWidth = container.querySelector('.featured-card')?.getBoundingClientRect().width || 0;
    const gap = 24;
      ...
      <div className="pl-8 md:pl-16">
        ...
        <div className="relative mx-auto px-4">
          ...
          {questions.map((question) => {
            return (
              <div key={question.id} className="featured-card flex-none w-[280px] h-[300px] snap-center">
                <Card className="border p-4 border-blue-400 rounded-lg hover:shadow-sm transition-all duration-200 bg-white overflow-hidden h-full">
                  <Link to={question.themeId ? `/themes/${question.themeId}/questions/${question.id}` : `/sharp-questions/${question.id}`} className="block h-full">
